...
databaseId: my-db            # 任意。指定しない場合は (default) データベースを使用。
...
- `--database-id <id>`: Firestore データベース ID（省略時は `(default)`）
...
# named database（非 (default) の Firestore DB）を対象に読み取る
...
databaseId: my-db                        # 任意。省略時は (default) Firestore DB を使用。
...
| `FIRESTORE_DATABASE_ID` | Firestore データベース ID（named database。省略時は `(default)`） |
| `FIRESTORE_EMULATOR_HOST` | Firestore エミュレータホスト（例: localhost:8080） |
| `FIRESTORE_DATABASE_URL` | Realtime Database の URL（`FIRESTORE_DATABASE_ID` とは別物。Realtime DB 利用時のみ必要） |
